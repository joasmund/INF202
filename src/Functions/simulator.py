...
def run_simulation(config_path, output_dir):
    ...
    try:
        ...
        try:
            ...
            # Mark border cells if borders are defined in config
            if "borders" in config.get("geometry", {}):
                borders = config["geometry"]["borders"]
                border_cells = mesh.mark_border_cells(borders)
                logger.info(f"Marked {len(border_cells)} cells within border region")
        ...
        border_oil_history = []  # Track oil in border cells over time
        ...
        for step in range(nSteps + 1):
            ...
            try:
                ...
                # Calculate and log border oil sum
                if hasattr(mesh, 'border_cells'):
                    border_oil_sum = mesh.calculate_border_oil_sum()
                    border_oil_history.append(border_oil_sum)
                    if step % writeFrequency == 0:
                        logger.info(f"Border region oil sum at step {step}: {border_oil_sum:.3f}")
                
                if step % writeFrequency == 0:
                    logger.info(f"Step {step}/{nSteps} completed (t={current_time:.3f})")
                    
                    # Create visualization with border cells highlighted
                    try:
                        triangulation = Triangulation(points[:, 0], points[:, 1], triangles)
                        plt.figure(figsize=(8, 6))
                        
                        # Create base color map for oil amounts
                        plt.tripcolor(triangulation, facecolors=updated_oil_amounts, 
                                    cmap="viridis", shading="flat")
                        
                        # Add overlay for bay cells
                        bay_mask = np.zeros(len(triangles), dtype=bool)
                        for i, cell_index in enumerate(triangle_cell_indices):
                            cell = mesh._cells[cell_index - 1]
                            if cell._cell_in_bay:
                                bay_mask[i] = True
                        
                        # Add darker overlay to bay cells
                        plt.tripcolor(triangulation, facecolors=bay_mask.astype(float),
                                    cmap='binary', alpha=0.3, shading="flat")
                        
                        plt.colorbar(label="Oil Amount")
                        plt.title(f"Oil Distribution at Step {step}\n(Dark overlay shows bay region)")
                        plt.xlabel("X")
                        plt.ylabel("Y")
                        
                        # Log bay oil amount
                        if step % writeFrequency == 0:
                            bay_oil_sum = mesh.calculate_bay_oil_sum()
                            logger.info(f"Bay region oil sum at step {step}: {bay_oil_sum:.3f}")
        ...
        # Add border oil history to simulation results
        if border_oil_history:
            simulation_data['border_oil_history'] = border_oil_history
            
        # Update the format_simulation_results function call to include border information
